...
#define BALL_WINDOW_X1 64
#define BALL_WINDOW_X2 1236
#define BALL_WINDOW_Y1 114
#define BALL_WINDOW_Y2 765
#define MIDDLE_OF_FIELD_Y 440
...
typedef struct game {
    ...
    Text *pStartText, *pWaitingText, *pOverText, *pMatchTimerText, *pGoalsTextTeamA, *pGoalsTextTeamB, *pPowerUpText[2];
            ...
            case ONGOING:
                ...
                if (!goal(pGame->pBall)) {
                    ...
                    if (!goalScored(pGame->pBall)) {
                        pGame->teamA++;
                    } else if (goalScored(pGame->pBall)) {
                        pGame->teamB++;
                    }
                }
                renderGame(pGame);
...
void handlePowerUpText(Game *pGame) {
    ...
    pGame->pPowerUpText[1] = createText(pGame->pRenderer, 238, 168, 65, pGame->pFont, "Speed increased!", WINDOW_WIDTH / 2, WINDOW_HEIGHT - 100);
